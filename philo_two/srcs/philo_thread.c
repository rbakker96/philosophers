...
/*   Updated: 2021/01/06 11:20:00 by roybakker     ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
{
	pthread_mutex_lock(&philo->mutex->forks[left_fork]);						//semaphore
	print_status(philo, "\tgot left fork\n", philo->id, timestamp(philo));
	pthread_mutex_lock(&philo->mutex->forks[right_fork]);						//semaphore
...
void	eating(t_philo *philo)
{
	...
	pthread_mutex_unlock(&philo->mutex->forks[fork[left]]);						//semaphore
	pthread_mutex_unlock(&philo->mutex->forks[fork[right]]);					//semaphore
}
